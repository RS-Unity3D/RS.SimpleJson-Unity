...
SimpleJson-Unity 是一个专为Unity优化的轻量级JSON序列化/反序列化库，基于facebook的Simple-Json项目改进而来，特别针对Unity的AOT（Ahead-Of-Time）编译环境进行了优化。
...
### 1. 复制文件
将 `SimpleJson-Unity` 复制到你的Unity项目中，建议放在 `Scripts/Utils/` 目录下。
...
SimpleJson-Unity 是一个专为Unity优化的轻量级JSON库，特别适合：
...
**文档版本**: 1.1.0.0  
**适用版本**: SimpleJson-Unity v1.1.0.0+
